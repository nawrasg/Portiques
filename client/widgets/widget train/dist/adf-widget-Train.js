...
ajs.controller('TrainWidgetController', ["$scope", "$http", "$rootScope", "$filter", function($scope, $http, $rootScope, $filter) {
...
angular.module("adf.widget.Train").run(["$templateCache", function($templateCache) {$templateCache.put("{widgetsPath}/Train/src/edit.html","<form role=form><div class=form-group><label for=sample>Num&eacute;ro Train</label> <input type=text class=form-control id=sample ng-model=config.train></div></form>");
$templateCache.put("{widgetsPath}/Train/src/view.html","<div ng-controller=TrainWidgetController><button type=button ng-class=statusClasse ng-click=status() id=status>{{statusLabel}}</button><div style=margin-bottom:30px></div><canvas id=doughnut class=\"chart chart-doughnut\" data=data labels=labels colours=colors options=options>test</canvas></div>");}]);})(window);